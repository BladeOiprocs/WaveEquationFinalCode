#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <math.h>



void runCalc(float***,int,int,int,float,float,float);
void outputData(FILE*,float***,int,int,int);

int main(int argc, char const *argv[]){

	int i,j,k;

	FILE* FPTR;
	char filename[16] = "dataSerial.csv";

    // MxNxO matrix
	//int M = 51;   //i
	int M = atoi(argv[1]);
	int N = M;   //j
	//int O = 100;  //k
	int O = atoi(argv[2]);

	// utt = c^2 (uxx + uyy)
	float initial_value = 10.0; //initial displacement
	float dt = 0.1;
	float dh = 0.16;
	float c = 1.0;

    // Define 3D Array
	float*** A = (float***)malloc(M*sizeof(float**));

    // Allocate Memory for 3D Array
	for(i=0; i<M; i++){
		A[i] = (float**)malloc(N*sizeof(float*));

		for(j=0; j<N; j++){
			A[i][j] = (float*)malloc(O*sizeof(float));
		}
	}

    // Assign Initial Values to Array
	for (i=0; i<M; i++){
		for (j=0; j<N; j++){
			for (k=0; k<O; k++){
				A[i][j][k]= 0.0;
			}
		}
	}
	// Set initial displacement at center of grid
	A[(M/2)][(N/2)][0] = initial_value;


	FPTR = fopen(filename,"w");

	char str;

    //Begin timer
    clock_t startTime, middleTime, endTime;
    double calcTime, totalTime;
    startTime = clock();

    // Do Calculations and Output Data
	for(k=1;k<O;k++){
		runCalc(A,M,N,k,dt,dh,c);
	}
	middleTime = clock();
	calcTime = ((double) (middleTime-startTime)/CLOCKS_PER_SEC);

	/*
	for(k=1;k<O;k++){
		outputData(FPTR,A,M,N,k);
	}
	*/

	fclose(FPTR);

    //end timer and calculate elapsed time
	endTime = clock();
	totalTime = ((double) endTime-startTime)/CLOCKS_PER_SEC;

	//print thread number, grid size, # of time steps, and real time elapsed
	printf("~~~ DETAILS ~~~\n");
	printf("Thread #:\t%d\n",1);
	printf("Grid size:\t%dx%d\n",M,N);
	printf("Calc Time:\t%3.4f\n",calcTime);
	printf("Total Time:\t%3.4f sec\n",totalTime);



// Free Memory
	for(i=0; i<M; i++){
		for(j=0; j<N; j++){
			free(A[i][j]);
		}
		free(A[i]);
	}
	free(A);


	return 0;

}


void runCalc(float*** A, int M,int N, int k, float dt, float dh, float c){
		int i,j;

		for(j=1;j<N-1;j++){
			for(i=1;i<M-1;i++){
				A[i][j][k+1] = 2.0*A[i][j][k]-A[i][j][k-1] + (pow(dt,2)*pow(c,2)/pow(dh,2))*(A[i+1][j][k]+A[i-1][j][k]+A[i][j+1][k]+A[i][j-1][k]-4.0*A[i][j][k]);
			}
		}

	return;
}

void outputData(FILE* FPTR, float*** A, int M, int N, int k){
    int i,j;
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			fprintf(FPTR,"%.6f,",A[i][j][k]);
		}
		fprintf(FPTR,"\n");
	}
	fprintf(FPTR,"\n");
}
